 ...
 * @Description: snowflake算法生成自增式主键
 *
 * @author: lingmeng.kong
 * @date: Jun 26, 2019 9:33:38 PM
 */
public class SnowFlake {
	private final long twepoch = 1420041600000L; // 开始时间截
	...
	private static SnowFlake single = null;
	...
	public static SnowFlake getInstance(long workerId, long datacenterId) {
		if (single == null) {
			synchronized (SnowFlake.class) {
				if (single == null) {
					single = new SnowFlake(workerId, datacenterId);
					return single;
				}
			}
		}
